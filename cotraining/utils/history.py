...
class History(torch.nn.Module):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, device='cpu'):
        ...
        self.emb = torch.zeros(num_embeddings, embedding_dim, device=device,
    ...
    @torch.no_grad()
    def push(self, x, n_id: Optional[torch.Tensor] = None,
             offset: Optional[torch.Tensor] = None, count: Optional[torch.Tensor] = None):
