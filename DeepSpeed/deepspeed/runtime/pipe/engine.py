...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        if self.grid.get_global_rank() == 0:
            logger.info(f"CONFIG: micro_batches={self.micro_batches} " f"micro_batch_size={self.micro_batch_size}")

        self.global_rank = self.grid.get_global_rank()

        assert self.dp_world_size == self.grid.data_parallel_size
        assert self.train_batch_size() == self.micro_batch_size * self.micro_batches * self.grid.data_parallel_size
        ...
        logger.info(
            f"is_pipe_partitioned= {self.is_pipe_partitioned} " f"is_grad_partitioned= {self.is_grad_partitioned}"
        )
    ...
    def _build_data_iter(self, dataset):
        sampler = torch.utils.data.distributed.DistributedSampler(
            dataset, num_replicas=self.dp_world_size, rank=self.mpu.get_data_parallel_rank(), shuffle=False
        )
    ...
    def train_batch(self, data_iter=None):
        ...
        if self.global_rank == 0 and self.monitor.enabled:
            self.summary_events = [
                (f"Train/Samples/train_loss", self.agg_train_loss.mean().item(), self.global_samples)
            ]
    ...
    def eval_batch(
        ...
        self._eval_mode = do_eval
        if last_batch:
            # reset for the next batch as the shape might change
            self.first_output_send = True 
            self.pipe_recv_buf = None
        ...
        if self.is_last_stage():
            ...
            if self.is_data_parallel:
                if self.agg_additional_losses is None:
                    ...
                    self.agg_additional_losses = OrderedDict(
                        {name: reduced_tensor[name] for name in self.agg_additional_losses.keys()}
                    )
            ...
            if additional_losses is not None:
                self.agg_additional_losses = OrderedDict(
                    {name: losses[2 + i].clone().detach() for i, name in enumerate(additional_losses.keys())}
                )
        return agg_loss
    ...
    def _exec_forward_pass(self, buffer_id):
        ...
        if self.is_last_stage():
            ...
            if self.eval_return_logits:
                if self._eval_mode:
                    labels = self.pipe_buffers["labels"][buffer_id]
                    eval_output = self.module.eval_fn(outputs, labels)
                    self.outputs = eval_output
                else:
    ...
    def _exec_optimizer_step(self, lr_kwargs=None):
        ...
        if self.global_rank == 0 and self.monitor.enabled:
            ...
            if self.fp16_enabled() and hasattr(self.optimizer, "cur_scale"):
                self.summary_events.append((f"Train/Samples/loss_scale", self.optimizer.cur_scale, self.global_samples))
            self.monitor.write_events(self.summary_events)
    ...
    def load_module_state_dict(self, checkpoint, strict=True, custom_load_fn=None, fetch_z3_params=False):
        ...
        self.module.load_state_dir(
            load_dir=self._curr_ckpt_path, strict=strict, checkpoint_engine=self.checkpoint_engine
        )
