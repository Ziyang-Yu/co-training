...
class FP16_UnfusedOptimizer(DeepSpeedOptimizer):
    ...
    def __init__(self,
        ...
        self.mpu = mpu
        group_size = self.mpu.get_model_parallel_world_size()
        rank_idx = self.mpu.get_model_parallel_rank()
        self.global_group_nums = torch.zeros(group_size, dtype=torch.int32, device=get_accelerator().current_device_name())
        self.global_group_nums[rank_idx] = len(self.fp16_groups)
        dist.all_reduce(self.global_group_nums, op=dist.reduce_op.ReduceOp.SUM, group=self.mpu.get_model_parallel_group())
        logger.info(f'rank={dist.get_rank()} global_group_nums={self.global_group_nums}')
        self.shared_group_num = self.global_group_nums.min().item()
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            norm_group_value = 0.0
            if len(grads_for_norm) > 0:
                norm_group_value = get_weight_norm(grads_for_norm, mpu=self.mpu if i > self.shared_group_num else None)
            norm_groups.append(norm_group_value)
